                ...
                <div class="swiper mySwiper custom_swiper">
                    <div class="swiper-wrapper">
                        <div class="swiper-slide"
                            style="background-image: url({{ asset('storage/app/' . $experience->image1) }});"></div>
                        <div class="swiper-slide"
                            style="background-image: url({{ asset('storage/app/' . $experience->image2) }});"></div>
                        <div class="swiper-slide"
                            style="background-image: url({{ asset('storage/app/' . $experience->image3) }});"></div>
                    </div>
...
<div class="container-fluid py-5 about_text_con" id="pagination-section">
    <div class="container">
        <div class="row">
            @foreach ($experienceDetail as $experience)
                <div class="col-xxl-6 col-xl-6 col-lg-6 col-md-6 col-sm-12 col-12 exp_div mb-4">
                    <div class="exp_img cover_bg"
                        style="background-image: url('{{ asset('storage/app/' . $experience->image1) }}');">
                    </div>
                    <div class="detail">
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination justify-content-end">
                    {{-- Previous Page Link --}}
                    @if ($experienceDetail->onFirstPage())
                    ...
                    @else
                        <li class="page-item">
                            <a class="page-link" href="{{ $experienceDetail->previousPageUrl() }}#pagination-section"
                                aria-label="Previous">
                                <span aria-hidden="true">&laquo;</span>
                            </a>
                        </li>
                    ...
                    {{-- Pagination Elements --}}
                    @foreach ($experienceDetail->links()->elements[0] as $page => $url)
                        @if ($page == $experienceDetail->currentPage())
                        ...
                        @else
                            <li class="page-item">
                                <a class="page-link"
                                    href="{{ $url }}#pagination-section">{{ $page }}</a>
                            </li>
                        @endif
                    @endforeach

                    {{-- Next Page Link --}}
                    @if ($experienceDetail->hasMorePages())
                        <li class="page-item">
                            <a class="page-link" href="{{ $experienceDetail->nextPageUrl() }}#pagination-section"
                                aria-label="Next">
